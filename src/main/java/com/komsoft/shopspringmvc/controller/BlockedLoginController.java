...
public class BlockedLoginController {
    ...
    public BlockedLoginController(UserService userService, CartService cartService) {
        ...
        if (count < MAX_TRY) {
            if (login != null && login.trim().length() != 0) {      // isBlank() is not supported
                try {
                    ...
                    if (user != null) {
                        modelMap.addAttribute(Header.MESSAGE, String.format(bundle.getString("accessGranted"), user.getFullName()));
                        session.setAttribute(Header.AUTHENTICATED_USER_KEY, user);
//      Get Cart if it isn't paid yet
                        CartModel cart = null;
                        try {
                            cart = cartService.getUnpaidCartByUser(user);
                            if (cart != null) {
                                session.setAttribute(Header.USER_CART, cart);
                            }
                        } catch (DataBaseException e) {
                            e.printStackTrace();    // Нема, так нема. Створимо нову, якщо юзер додасть товар
                        }
...
//          AUTHENTICATED_USER_KEY = userDto (fullName) from doPost()
            view = Header.LOGIN_SUCCESS_PAGE;
        }
            return view;
    }
